...
wrapper.init = function () {
    //com.ajax({ type: 'GET', url: 'api/sys/menu', success: function () { return;} });
    ...
    com.dialog({
        ...
        viewModel: function (w) {
            ...
            w.find("#pwd_confirm").click(function () {
                var oldPwd = w.find("[name=OldPwd]").val();
                var pwd = w.find("[name=Pwd]").val();
                var secondPwd = w.find("[name=SecondPwd]").val();
                if (pwd == '' || pwd == null) {
                    com.message('warning', '密码不能为空');
                    return;
                }
                if (pwd != secondPwd) {
                    com.message('warning', '两次输入的密码不一致');
                    return;
                }
                com.ajax({
                    url: '/api/sys/user/edituserpwd',
                    data: JSON.stringify({ Pwd: pwd, OldPwd: oldPwd }),
                    success: function (d) {
