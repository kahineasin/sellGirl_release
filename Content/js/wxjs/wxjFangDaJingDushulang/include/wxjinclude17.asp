...
do while not rs.eof
   ...
   '<a href='produceview.asp?sortid="&rs("sortid")&"&id="&rs("id")&"&sortpath="&request("sortpath")&"'>
	response.Write ("<div class='divBooklistC1L'><div class='divBooklistC1LImg'><a title='"&ProductName&"' href='"& replace(rs("smallpic"),"../","")&"'><img src='"& replace(rs("smallpic"),"../","")&"'/></a></div><div class='divBooklistC1LC'><a href='produceview.asp?sortid="&rs("sortid")&"&id="&rs("id")&"&sortpath="&request("sortpath")&"'>"&ProductName&"</a></div></div>")
	rs.movenext
	If rs.eof Then
		response.Write ("</div>")
	Else
...
function cgalview()
  ...
  if picid="" then
    ...
    strPicSource=picid
	 ...
	 response.Write "<div id='content'><h2 id='header'>Where are the Pirates of the Caribbean?</h2><div style='float: left; width:356px; height:205px; background:url("&strPicSource&") no-repeat; border:1px solid gray; margin-right: 1em; margin-bottom: 0.25em;'><img id='caribbean' onLoad='initLoupe(this.id,true,773,364);' src='"&strPicSource&"' style='cursor:wait;' width='500' height='300' alt='large image' border='0' /></div></div>"
...
set rs = server.createobject("adodb.recordset") 
  ...
  rs.open sql,conn,1,1
   ...
   while not rs.eof
	...
	response.Write "<li><a class='pro_a' ptxt='"&rs("content")&"' pname='"&rs("productname")&"' psrc='"&rs("bigpic2")&"' phref='productshow.asp?sid="&rs("sortid")&"&id="&rs("id")&"'><img style='width:119px;height:70px;' src='"&rs("smallpic")&"' onclick='document.getElementById(""idProShowPic"").src=this.src;location.reload();'/></a></li>"
...
function bbsindexannounce(getSortId)
	...
	Else
		For i=1 to 4
			...
			Response.Write("<li><a href=''>"&NewsName&"</a></li>")
			rs.movenext
		Next
...
set rs = server.createobject("adodb.recordset") 
  ...
  rs.open sql,conn,1,1
   ...
   while not rs.eof
	...
	response.Write "<li><a class='pro_a' ptxt='"&rs("content")&"' pname='"&rs("productname")&"' psrc='"&rs("BigPic")&"' phref='productshow.asp?sid="&rs("sortid")&"&id="&rs("id")&"'><img style='width:510px;height:350px;' src='"&rs("BigPic")&"' onclick='document.getElementById(""idProShowPic"").src=this.src'/></a></li>"
...
Function indexadimg()
	...
	Else
		...
		Do While Not rs.eof
			Response.Write("<a href='1234' target='_blank'><img src='"&rs("BigPic")&"' width='988' height='383' /><p style='display:none;'>"&rs("NewsName")&"</p></a>")
			rs.movenext
		Loop
		...
		'Response.write("<a href='1234' target='_blank'><img src='"&rs(strSlidePic1)&"' width='988' height='383' /><p>"&rs("ProductName")&"</p></a>")
		'Next
	End If
	rs.close
	Set rs=nothing
	...
	If IsEmpty(request.Form("nameSubmit"))=False Then
		...
		Else
			If rs("Password")=password Then
				...
				rs("money")=money+10
				rs.update
				response.Write("<script type='text/javascript' language='javascript'>alert('欢迎你!"&Session("memName")&"1111');</script>")
			Else
				response.Write("<script type='text/javascript' language='javascript'>alert('用户名错误');</script>")
			End If
		End If
