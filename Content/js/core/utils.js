...
* 模块名：共通脚本
* 程序名: 通用工具函数
* Copyright(c) 2013-2015 SellGirl 
**/
...
utils.fnValueToText = function (list, value, text) {
    ...
    for (var i in list) {
        ...
        //为了解决guid大小写问题
        map[list[i][value || 'value'].toLowerCase()] = list[i][text || 'text'];
    }
...
function UrlCode() {
    this.aCode = [];
    this.aCode.push({ 'Mark': '&', 'Num': '26','En':'&' });
    this.aCode.push({ 'Mark': '<', 'Num': '3c','En':'lt'});
    this.aCode.push({ 'Mark': '>', 'Num': '3e','En':'gt'});
}
UrlCode.prototype.showCode = function () {
    //console.info(this.aCode);
    for (var i in this.aCode) {
        alert(this.aCode[i].Mark);
    }
}
/**
* 把字符串转换为单精度
* 用法:utils.toUrlEncoded(v,model) 
* 参数:model,0:单精度,1:双精度;
*/

utils.urlEncode = function (v) {
    var sDouble = "";
    ...
    var oUrlCode = new UrlCode();
    //oUrlCode.showCode();
    for (var i in oUrlCode.aCode) {
        v = v.replace(eval("/\\"+oUrlCode.aCode[i].Mark+"/g"), "%" + sDouble + oUrlCode.aCode[i].Num);//转译&符号防能从url传递
    }
    //v = v.replace(/\&/g, "%" + sDouble + "26");//转译&符号防能从url传递
    //v = v.replace(/\</g, "%" + sDouble + "3c");//转译&符号防能从url传递
    //v = v.replace(/\>/g, "%" + sDouble + "3e");//转译&符号防能从url传递
    return v;
}

utils.urlDecode = function (v) {
    var sDouble = "";
    if (arguments.length > 1) {
        if (arguments[1] == "1") {
            sDouble = "25";
    ...
    //oUrlCode.showCode();
    for (var i in oUrlCode.aCode) {
        v = v.replace(eval("/\\%" + sDouble +oUrlCode.aCode[i].Num+ "/g"), oUrlCode.aCode[i].Mark);//转译&符号防能从url传递
    }
    //v = v.replace(eval("/\\%" + sDouble + "26/g"), "&");//转译&符号防能从url传递
    //v = v.replace(eval("/\\%" + sDouble + "3c/g"), "<");//转译&符号防能从url传递
    //v = v.replace(eval("/\\%" + sDouble + "3e/g"), ">");//转译&符号防能从url传递
    return v;
}

utils.htmlEncode = function (v) {
    var oUrlCode = new UrlCode();
    //oUrlCode.showCode();
    for (var i in oUrlCode.aCode) {
        v = v.replace(eval("/\\" + oUrlCode.aCode[i].Mark + "/g"), "&" + oUrlCode.aCode[i].En+";");//转译&符号防能从url传递
    }
    return v;
}

utils.htmlDecode = function (v) {
    var oUrlCode = new UrlCode();
    //oUrlCode.showCode();
    for (var i in oUrlCode.aCode) {
        v = v.replace(eval("/\\&" + oUrlCode.aCode[i].En + ";/g"), oUrlCode.aCode[i].Mark);//转译&符号防能从url传递
    }
    return v;
}