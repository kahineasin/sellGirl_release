...
com.message = function (type, message, callback) {
    switch (type) {
        case "success":
            if (parent == window) {
                if ($('#notity').length > 0) {
                    return $('#notity').jnotifyAddMessage({ text: message, permanent: false });
                }
                return alert(message);
            }
            parent.$('#notity').jnotifyAddMessage({ text: message, permanent: false });
            break;
        case "sms"://表示发送通知短信失败,但是数据更新成功
        ...
        case "warning":
            if (parent == window) {
                if ($('#notity').length > 0) {
                    return $('#notity').jnotifyAddMessage({ text: message, permanent: false, type: 'warning' });
                }
                return alert(message)
            };
...
com.auditDialog = function () {
    ...
    //if (winAudit.length == 0) {
        ...
        //winAudit = wrapper.find(".easyui-dialog").dialog({//by wxj
        //    onClose: function () {
        //        ko.cleanNode(node);
        //        //parent.$('#w_sendinstall_confirm_div').remove();
        //        parent.$('#w_audit_div').remove();
        //    }
        //});
    //}
...
com.auditDialogForEditVM = function () {
    ...
    var viewModel = function () {
        ...
        this.confirmClick = function () {
            ...
            //ko.cleanNode(winAudit.parent()[0]);//这样才能防止崩溃,解决innerHeight报错
        };
        this.cancelClick = function () {
            winAudit.dialog('close');
        };
