...
(function ($) {
            ...
            that.bind("mousedown.autocomplete", function (e) {
                ...
                switch (e.keyCode) {
                    ...
                    case 40: //小键盘下箭头
                        ...
                        break;
                    case 13: //Enter键
                        if (($.data(e.currentTarget, "autocomplete")).panel.is(':visible') === true) {//如果下拉面板未展开,不执行.为解决按Enter时不触发最外层form的submit按钮的点击事件的bug,例子Product的Index页--wxj20180127
                            e.preventDefault();
                            hidePanel(jq);
                            return false;
                        }
    ...
    //对'_'下划线的like似乎不正确,也可能是查数据库时的错误 --wxj
    function formatItem(cols, count, q, w) {
        ...
        switch (count) {
            case 1:
                ...
                //更改为可以从中间开始like--wxj
                index = cols[0].indexOf(q); 
                html = utils.formatString('<strong>{0}</strong>{1}<strong>{2}</strong>', cols[0].substr(0, index), cols[0].substr(index, q.length), cols[0].substr(q.length+index, cols[0].length));
                break;
            default:
                ...
                for (var m in cols) {
                    ...
                    //更改为可以从中间开始like--wxj
                    index = cols[m].indexOf(q);
                    var strong = (cols[m].substr(0, q.length) == q) ? "<strong>{0}</strong>{1}<strong>{2}</strong>" : "{0}{1}{2}";
                    var text = utils.formatString(strong, cols[m].substr(0, index), cols[m].substr(index, q.length), cols[m].substr(q.length + index, cols[0].length));
    ...
    function findComboItem(panel, val) {////如果value的值如 http://bbs.3dmgame.com 时会报错,所以在外面加单引号.多个地方调用,所以提取到这里--wxj20180127
        return panel.find("div.combobox-item[value='" + val + "']");
    }
    /**
    * 小键盘向上选择操作
    * @param {Object} jq
    */
    function selectPrev(jq) {
        ...
        var item = findComboItem(panel, that.val());//wxj20180127
        if (item.length) {
    ...
    */
    ...
    function scrollTo(jq, value) {
        ...
        var item = findComboItem(panel, value);//wxj20180127
        if (item.length) {
